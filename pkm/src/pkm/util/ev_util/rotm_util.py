...
def Rm2q(Rm):
    ...
    q = torch.zeros(Rm.shape[:-2]+(4,), dtype=Rm.dtype, device=Rm.device)
...
def qlog(q):
    ...
    abssinalpha = torch.clip(torch.abs(sinalpha), min=1e-6)
...
def safe_norm(x, axis, keepdims=False, eps=0.0):
    is_zero = torch.all(torch.isclose(x,0.), dim=axis, keepdim=True)
    # temporarily swap x with ones if is_zero, then swap back
    x = torch.where(is_zero, torch.ones_like(x), x)
    n = torch.linalg.norm(x, axis=axis, keepdim=keepdims)
...
def custom_rotm(l, R, rot_configs):
    if l==0:
        return torch.tensor([[1.]], dtype=R.dtype)
    if l==1:
        return R
    w = q2aa(Rm2q(R))
    return torch.linalg.matrix_exp(torch.sum(w[...,None,None] * rot_configs['Js'][l], dim=-3))
...
def make_Rp(xin, base_axis='z', normalize_input=True):
    if normalize_input:
        mag = torch.linalg.norm(xin, dim=-1, keepdim=True)
    else:
        mag = 1
    if base_axis=='z':
        z = xin/mag
...
def Y_func(l, xin, rot_configs):
    if l==1:
        return xin
    mag = torch.linalg.norm(xin, axis=-1, keepdims=True)
    if l==0:
        return mag
...
def Y_func_V2(l, x, rot_configs, normalized_input=False):
    ...
    if normalized_input:
        ...
        mag = torch.linalg.norm(x, dim=-1, keepdim=True) + 1e-6
        xhat = x/mag
    if l==0:
        return mag
    xxhat = (xhat[...,None]*xhat[...,None,:]).reshape(*xhat.shape[:-1],-1)
    xhat_blinear = torch.cat([xhat*xhat, xxhat], dim=-1) # for order 2
    if l==2:
...
def calculate_rotm_basis(rot_configs, rng):
    ...
    return [
        None,
        None,
        torch.DoubleTensor(WDCOEF[2]),
        torch.DoubleTensor(WDCOEF[3]),
    ]
...
def apply_rot(x, R, rot_configs, feature_axis=-2, expand_R_no=None):
    x = torch.swapaxes(x, feature_axis, -1)
    ...
    return torch.swapaxes(torch.cat(xr, dim=-1), feature_axis, -1)
...
def test_equivariance(seed):
    ...
    for ii in range(2,5):
        ...
        x = torch.randn(test_n, 3, generator=generator, dtype=torch.float64)
        rand_R1 = torch.DoubleTensor(rand_matrix(test_n))
        rand_R2 = torch.DoubleTensor(rand_matrix(test_n))
        ...
        assert torch.abs((y1-y2).mean(-1)).max()<3e-4

        # %%
        # D(I) = I
        resD = custom_rotmV2(i, torch.eye(3, dtype=torch.float64), rot_configs)
        residual = resD - torch.eye(resD.shape[-1], dtype=torch.float64)
        ...
        print(torch.count_nonzero(torch.any(torch.any(torch.abs(residual)>5e-4, axis=-1), axis=-1)))
        assert torch.all(torch.abs(residual)<5e-4)

        # %%
        # det
        detres = torch.linalg.det(resD1)
        residual = detres-1
        print(torch.count_nonzero(torch.abs(residual)>5e-4))
        assert torch.all(torch.abs(residual)<5e-4)
        ...
        residual = D1D1T - torch.eye(D1D1T.shape[-1])
        print(torch.count_nonzero(torch.abs(residual)>5e-4))
        assert torch.all(torch.abs(residual)<5e-4)
